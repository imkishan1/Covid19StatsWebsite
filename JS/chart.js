async function getcoviddata() {
  ...
  sevendayavgmort.innerText = `${avgmort.replace(
    /(\d)(?=(\d\d)+\d$)/g,
    "$1,"
  )}`;

  let sevendayavgrecov = document.getElementById("recov");
  sevendayavgrecov.innerText = `${avgrecov.replace(
    /(\d)(?=(\d\d)+\d$)/g,
    "$1,"
  )}`;
  ...
  for (var i = 2; i < testlen; i++) {
    ...
    if (graphdata.tested[i].totaldosesadministered == "") {
      datavaccine =
        graphdata.tested[i - 1].totaldosesadministered -
        graphdata.tested[i - 2].totaldosesadministered;
      ...
      datavaccine =
        graphdata.tested[i].totaldosesadministered -
        graphdata.tested[i - 1].totaldosesadministered;
      ...
      datasets: [
        {
          ...
          data: vaccinelast,
          backgroundColor: ["rgba(219, 85, 129,0.2)"],
          borderColor: ["rgba(219, 85, 129,1)"],
          borderWidth: 1,
        },
      ],
    },
